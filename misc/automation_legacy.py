...
# from src.update_task import UpdateTask
...
def get_launcher_data():
    ...
    # endpoints = ["DMA Launcher - AGT/", "DMA Launcher - IST/", "DMA Launcher - MGT/", "DMA Launcher - RCE/", "DMA Launcher - STC/", "DMA Launcher - SGE/", "DMA Launcher - RLA/", "DMA Launcher - LDA/", "DMA Launcher - EDGE/", "DMA Launcher - Test/", "SGT Agent Launcher"]
    endpoints = settings.launcher_details['legacy_launcher_list']
    launcher_ids = []
    launcher_descriptions = []
    print(endpoints)
    for endpoint in endpoints:
        product_line = endpoint.replace("DMA Launcher - ", "").replace(" Agent Launcher", "").rstrip("/")
        log_info(f"Productline Legacy: {product_line}", timestamp_value)
        ...
        if "id" in launcher_data:
            ...
            log_info(f"Legacy Launcher Id:, {launcher_id}", timestamp_value)
            log_info(f"Legacy Launcher Description:, {launcher_id}", timestamp_value)

        else:
            # pass
            log_error(f"Launcher data not found for {product_line}.", timestamp_value)
...
def get_assembly_details():
    ...
    log_info(f"Legacy Assembly details are:", timestamp_value)
    ...
    if tag_types_response.status_code == 200:
        ...
        for assembly_id in assembly_ids_integrated:
            ...
            if response.status_code == 200:
                data = response.json()
                package_name = next((pkg['Package Name'] for pkg in package_details if pkg['Package ID'] == package_id),
                                    None)
...
for package_name, package_data in result.items():
    log_info(f"Legacy Package Details: ", timestamp_value)
...
def get_event_data(package_details):
    ...
    for package in package_details:
        ...
        events_url = f"{events_endpoint}?packageId={package_id}&from={settings.conf_data['from_date']}&to={settings.conf_data['to_date']}&tagIdentifierOption=TagName"
...
def generate_output_data(package_details, agent_ids):
    ...
    for package in package_details:
        ...
        package_data = {
            ...
            'Package Name': package_name,
            'Package ID': package_id,
            'Signal Data': get_signal_data_24h(tag_ids) if num_of_days < 1 else get_signal_data_long_date_test(tag_ids,
                                                                                                               assembly_ids_integrated),
...
try:
    ...
    log_success(f'Signal Data generated for Legacy DMA', timestamp_value)
    log_success(f'Event Data generated for Legacy DMA', timestamp_value)
    log_success(f'Agent Messages Data generated for Legacy DMA', timestamp_value)
...
log_info(f"Execution time Legacy:  {execution_time} \"seconds\"", timestamp_value)
